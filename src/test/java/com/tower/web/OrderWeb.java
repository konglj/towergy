...
import com.tower.common.util.Constants;
...
public class OrderWeb {
	...
	public Object orderThreeHt(HttpServletRequest request, OrderInfo orderInfo,
		...
		try {
			...
			String threePath=new File(templatePath).getParent()+"\\"+Config.getTowerfile()+"\\images\\order\\ht\\";
	...
	@ResponseBody
	public Object addOrder(HttpServletRequest request, int towerid,String userwxid) {

		if(towerid==0||userwxid==null)
			return ResultUtil.generateResponseMsg("error","派单失败！");
				Map map = new HashMap();
				map.put("towerid", towerid);
				Tower tower = towerService.getTowerInfo(towerid);
				if (tower == null || tower.getTowerstate()!=0)
					return ResultUtil.generateResponseMsg("error","站址不存在或已被抢单！");
				// 判断进行中状态，以前按照tower_user_order_info表中的ingcount
				// 现在获取tower_order_detail 中的预付款审核通过之前的订单数量

				// 先获取站点状态
				// int state=towerService.getTowerState(Integer.valueOf(towerid));
				int state = tower.getTowerstate();

				UserInfo userInfo=userService.getUserdetail(userwxid);
				if(userInfo==null){
					return ResultUtil.generateResponseMsg("error","读取用户信息失败！");
				}
				Order order = new Order();
				String orderid = ParamerUtil.getOrderid(userInfo.getId(), StrUtil.qd);
				order.setUserwxid(userwxid);
				order.setTowerid(towerid);
				order.setTowerstate(0);
				order.setOrderid(orderid);
				order.setQdtime(Config.getQdtime() * 24);
				order.setTowerfee(tower.getTowerfee());

				int count = 0;
				SysUserInfo sysUserInfo = ParamerUtil.getSysUserFromSesson(request);
				try {
					count = orderService.insertOrder(order,sysUserInfo.getId());
