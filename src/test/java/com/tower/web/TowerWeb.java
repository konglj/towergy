...
public class TowerWeb {
	...
	public ModelAndView getTowerYes(HttpServletRequest request,
		...
		if (queryTerms != null) {
			...
			towersInputData.setTowerstore(queryTerms.getTowerstore());
			towersInputData.setIstowerinfo(queryTerms.getIstowerinfo());
			// 当将todate修改后，还原
		...
		towersPage.setTowerstores(towerService.getTowerStores());
	...
	public ModelAndView getTowerNo(HttpServletRequest request,
		...
		noTowersPage.setTowerstores(towerService.getTowerStores());
	...
	public Object applyAddTower(HttpServletRequest request, AddTowerInfo tower) {
		...
		// 系统编号
		try {
			String towerid = towerService.getAreaOrder(tower.getTowerarea(),
					0);
	...
	public Object towerUpload(HttpServletRequest request) {
		...
		for (AddTowerInfo addTowerInfo : towers) {
			...
			if(addTowerInfo.getCityname()==null){
				return ResultUtil.generateResponseMsg("error", "站址区域错误不能为空");
			}
			City city = areaService.getCityByName(addTowerInfo.getCityname().replace(" ",""));
			if (city == null || city.getId() == 0)
				return ResultUtil.generateResponseMsg("error", "站址区域错误--"+addTowerInfo.getCityname());
			// 判读区域范围
			// 如果为城市管理员。则获取城市名称

			// 区域
			
				if (!ParamerUtil.isArea(citys, city.getId()))
					return ResultUtil.generateResponseMsg("error",
							"站址包含不是管辖范围的站址！");
			
			// 街道

			addTowerInfo.setTowerarea(city.getId());
			// 根据站址类型获取站址类型ID
			...
			// 获取towerid
			String towerid = towerService.getAreaOrder(city.getId(),0);
	...
	@RequestMapping("/update_tower_img")
	@ResponseBody
	public Object uploadTowerInfoImg(HttpServletRequest request){
		String filepath = "\\images\\tower\\towerinfo\\";
