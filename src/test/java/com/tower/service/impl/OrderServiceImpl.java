...
public class OrderServiceImpl implements OrderService {
	...
	public int shOrder(Map map) {
		...
		if (updatetowerstate == 3 || updatetowerstate == 18) {
			...
			if(order.getTowerinfostate()==2){
				  map.put("towervisible", Config.towerPaiDanRecvery);
			}
		...
		try {
			...
			if (updatetowerstate == 3 || updatetowerstate == 18) {
				if(order.getTowerinfostate()==2){
					...
					mapArea.put("powers", new String[]{"3","13"});
					mapArea.put("city", order.getTowercityid());
					sendMsg(Integer.valueOf(order.getId()), mapArea, WXInfo.task_pd_order_cancel_title,
							WXInfo.task_pd_order_cancel_name, WXInfo.task_pd_order_cancel_type,
							WXInfo.task_pd_order_cancel_remark);
				}
			}
	...
	@Override
	@Transactional
	public int updateTowerRentHtAply(int adminid, String shinfo,
	...
	@Override
	@Transactional
	public int updateTowerThreeHtAply(String templatePath,String threePath,int adminid, String shinfo,
			OrderInfo orderInfo, int adminpower) {
		// 根据订单获取订单状态
		OrderTower order = orderMapper.getOrderTower(orderInfo.getId());
		if (order == null)
		...
		String imagePath="\\"+Config.getTowerfile()+"\\images\\order\\ht\\"+filename;
	...
	@Override
	@Transactional
	public int insertOrder(Order order,int adminid) {
		Map map=new HashMap();
		map.put("towervisible",1);
		map.put("towerstate",2);
		map.put("towerid", order.getTowerid());
		int count=towersMapper.updateTowerPaiDan(map);
		if(count==0)
			return 0;
		...
		// 插入插入操作日志表
				...
				shInfo.setOrderuser(userInfo.getAdminname());
				shInfo.setOrderphone(userInfo.getAdminphone());
				shInfo.setOrderstate(25);
				shInfo.setOrdertype(userInfo.getAdminpower());
				shInfo.setOrdershinfo((String) map.get("shinfo"));
				count = towerShInfoMapper.insertTowerShInfo(shInfo);
